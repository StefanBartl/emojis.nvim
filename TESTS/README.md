...
| `search_spec.lua`   | `build_cmd()` glob/no_ignore flags; `apply_across_files()` confirm-gated cwd clear/replace. (The output-chunk line splitting moved to `lib.nvim.system.lines`, which owns its own spec.) |
