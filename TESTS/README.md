...
| `search_spec.lua`   | `build_cmd()` glob/no_ignore flags; `line_collector()` rejoining a line split across output chunks and stripping a CRLF file's trailing CR; `apply_across_files()` confirm-gated cwd clear/replace. |
